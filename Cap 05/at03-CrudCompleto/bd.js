...
const obterChavePrimaria = async (tabela) => {
    ...
    if (result.rows.length === 0) {
        throw new Error(`A tabela '${tabela}' não possui chave primária definida ou não existe.`)
    }
...
export const inserir = async (tabela, dados) => {
    ...
    if (!info.semPK || info.semPK.length === 0) {
        throw new Error(`Nenhum campo encontrado para inserir na tabela '${tabela}'.`)
    }

    const campos = info.semPK.join(', ')
    const placeholders = info.semPK.map((_, i) => `$${i + 1}`).join(', ')

    const sql = `INSERT INTO ${tabela} (${campos}) VALUES (${placeholders}) RETURNING *`
    const valores = info.semPK.map(campo => dados[campo])

    // Log de segurança para debugar o que está sendo enviado ao banco
    console.log("SQL GERADO:", sql)
    console.log("VALORES:", valores)

    if (!sql || sql.trim() === "") {
        throw new Error("A query SQL gerada está vazia!")
    }
